...
import com.jifenke.lepluslive.merchant.domain.entities.MerchantLicenseType;
import com.jifenke.lepluslive.merchant.domain.entities.MerchantProtocolType;
...
public class MerchantImageService {
  ...
  public void saveImage(MultipartFile fileData, String filePath, Integer type, Long imgType,
    ...
    if (type == 0) {
      ...
      MerchantLicenseType licenseType = new MerchantLicenseType();
      licenseType.setId(imgType);
      merchantLicense.setLicenseType(licenseType);
    ...
    if (type == 1) {
      ...
      MerchantProtocolType protocalType = new MerchantProtocolType();
      protocalType.setId(imgType);
      merchantProtocol.setProtocolType(protocalType);
  ...
  public BussineProtocalDto getProtImg(Long merchantId) {
    ...
    for (MerchantProtocol mp : list) {
      if (mp.getProtocolType().getId() == 1) {                //  1-平台受理服务协议书
        if (mp.getPicIndex() == 0) {
      ...
      if (mp.getProtocolType().getId() == 2) {               //  2-商户基础资料表
        if (mp.getPicIndex() == 0) {
      ...
      if (mp.getProtocolType().getId() == 3) {               // 3-中汇支付收单特约商户信息调查表
        if (mp.getPicIndex() == 0) {
      ...
      if (mp.getProtocolType().getId() == 4) {                // 4-结算授权书
        if (mp.getPicIndex() == 0) {
  ...
  public BussineLicenseImgDto getLiceImg(Long merchantId) {
    ...
    for (MerchantLicense lic : list) {             // 图片类型：
      ...
      if (lic.getLicenseType().getId() == 1) {       // 1-营业执照
        licDto.setLicenseImg(lic.getPicture());
      }
      if (lic.getLicenseType().getId() == 2) {       // 2-法人身份证
        licDto.setIdCardImg(lic.getPicture());
      }
      if (lic.getLicenseType().getId() == 3) {       // 3-税务登记证
        licDto.setTaxRegistrationImg(lic.getPicture());
      }
      if (lic.getLicenseType().getId() == 4) {       // 4-结算银行卡持有身份证
        licDto.setBankIdCardImg(lic.getPicture());
      }
      if (lic.getLicenseType().getId() == 5) {       // 5-组织结构照
        licDto.setOrgConstructionImg(lic.getPicture());
      }
      if (lic.getLicenseType().getId() == 6) {       // 6-结算银行卡
