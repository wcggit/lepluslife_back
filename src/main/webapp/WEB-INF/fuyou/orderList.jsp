...
<script>
    ...
    function showInfo(orderSid) {
        ...
        $.ajax({
                   ...
                   success: function (data) {
                       ...
                       $('#d-refundTime').html(new Date(order.completeDate).format('yyyy-MM-dd HH:mm:ss'));
        ...
        $.ajax({
                   ...
                   success: function (data) {
                       ...
                       for (var i = 0; i < content.length; i++) {
                           ...
                           contentStr += '<td>' + content[i].merchantUserId + '</td>';
                           ...
                           var typeDetail = '';
                           switch (eval(content[i].orderType.id)) {
                               case 12001:
                           ...
                           contentStr += '<td>' + payment + '</td>';
                           contentStr +=
                           '<td>￥' + content[i].totalPrice / 100 + '（￥' + content[i].truePay / 100
                           + '+￥' + content[i].trueScore / 100 + '红包）</td>';
                           contentStr +=
                           '<td>￥' + content[i].commission / 100 + '（￥'
                           + content[i].truePayCommission / 100
                           + '+￥' + content[i].scoreCommission / 100 + '红包）</td>';
                           contentStr +=
                           '<td>￥' + content[i].transferMoney / 100 + '（￥'
                           + content[i].transferMoneyFromTruePay / 100
                           + '+￥' + content[i].transferMoneyFromScore / 100 + '红包）</td>';
    ...
    function searchOrderByCriteria() {
        ...
        if ($("#merchant-userId").val() != "" && $("#merchant-userId").val() != null) {
            scanCodeOrderCriteria.merchantUserId = $("#merchant-userId").val();
        } else {
            scanCodeOrderCriteria.merchantUserId = null;
        }
        if ($("#orderType").val() != -1) {
