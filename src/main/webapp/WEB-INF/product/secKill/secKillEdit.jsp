...
<div id="content">
    ...
    <div class="m-right">
        <p style="padding-top: 20px">
            <button onclick="goOverviewPage()">返回秒杀概览</button>
            创建秒杀商品
        </p>
        <input id="productId" type="hidden" value="${product.id}"/>
        <input id="psk_id" type="hidden" value="${psk_id}"/>

        <div>
            <div>秒杀时段</div>
            <select class="form-control" id="pskTime" style="width: 60%">
                <c:forEach items="${list_t}" var="pskTime">
                    <option value="${pskTime.id}" <c:if test="${pskTime.id==edit_psktId}"> selected="selected" </c:if> >
                        ${pskTime.secKillDate}&nbsp;&nbsp;${pskTime.startTime}-${pskTime.endTime}</option>
                </c:forEach>
            </select>
        </div>
        <div>
        ...
        <div>
            <div>商品副标题</div>
            <input type="text" id="name2" class="check" value="${product.description}"/>
        </div>
        <div>
        ...
        <div>
            <div>产品关联</div>
            <div class="limited">
                <c:if test="${isLinkProduct == 0}">
                    <div>
                        <input type="radio" name="isLinkProduct" value="1">
                            <span>关联商城产品</span>
                        <div>
                            <span>关联商品序号</span><input name="linkProductId" value="">
                        </div>
                    </div>
                    <div>
                        <input type="radio" name="isLinkProduct" value="0" checked="checked" class="checked222">
                            <span>不关联商城产品</span>
                    </div>
                </c:if>
                <c:if test="${isLinkProduct != 0}">
                    <div>
                        <input type="radio" name="isLinkProduct" value="1" checked="checked" class="checked222">
                            <span>关联商城产品</span>
                        <div>
                            <span>关联商品序号</span><input name="linkProductId" value="${linkProduct.id}">
                        </div>
                    </div>
                    <div><input type="radio" name="isLinkProduct" value="0"><span>不关联商城产品</span></div>
                </c:if>
            </div>
        </div>
...
<script>
    ...
    $(function () {
        ...
        $("input[name=isLinkProduct]").click(function (e) {
            $("input[name=isLinkProduct]").removeClass("checked222");
            $(this).attr("class", "checked222");
        });
        $("input[name=isBackRed]").click(function (e) {
            $("input[name=isBackRed]").removeClass("checked333");
            $(this).attr("class", "checked333");
        });
        $("input[name=backRedType]").click(function (e) {
            $("input[name=backRedType]").removeClass("checked444");
            $(this).attr("class", "checked444");
        });
...
<script>
    ...
    $("#save").click(function () {
        var productSecKill = {}, productSecKillTime = {}, product = {}, productSpecList = [], scrollPictureList = [], productDetailList = [], goOn = 0;
...
//        alert(vals);
        ...
        //秒杀时段
        var pskTime = $("#pskTime").val();
        if (pskTime == "") {
            alert("请选择，秒杀时段");
            return
        }
        productSecKillTime.id = pskTime;
        productSecKill.productSecKillTime = productSecKillTime;
        //商品名称
        if ($("#name").val() == "") {
            alert("请输入商品名称");
            $("#name").focus();
        ...
        //产品关联
        var isLinkProduct = $(".checked222").val();
        if (isLinkProduct == null) {
            alert("请选择兑换限制");
            return
        } else if (isLinkProduct == 0) {
            productSecKill.isLinkProduct = isLinkProduct;
        } else if (isLinkProduct == 1) {
            var linkProductId = $("input[name=linkProductId]").val();
            if (linkProductId == "") {
                alert("请输入关联商品序号");
                $("input[name=linkProductId]").focus();
                return
            } else {
                var linkProduct = {};
                linkProduct.id = linkProductId;
                productSecKill.linkProduct = linkProduct;
                productSecKill.isLinkProduct = isLinkProduct;
            }
        }
        //是否返红包
        var isBackRed = $(".checked333").val();
        product.isBackRed = isBackRed;
        if (isBackRed == 1) {
            var backRedType = $(".checked444").val();
            product.backRedType = backRedType;
            if (backRedType == 1) {
                var backRatio = $("input[name=backRatio]").val();
                if (backRatio == "" || backRatio == null) {
                    alert("请输入返还比例");
                    $("input[name=backRatio]").focus();
                    return
                } else {
                ...
                if (backMoney == "" || backMoney == null) {
                    alert("请输入返还金额");
                    $("input[name=backMoney]").focus();
                    return
                } else {
                    product.backMoney = backMoney * 100.0;
                }
            } else {
                alert("请选择返红包类型");
                return
        ...
        console.log('--------------');
        productSecKill.id = $("#psk_id").val();
        var productDto = {};
        productSecKill.product = product;
        productDto.productSecKill = productSecKill;
        ...
        $.ajax({
                   type: "post",
                   url: "/manage/productSecKill/save",
                   contentType: "application/json",
                   data: JSON.stringify(productDto),
                   success: function (data) {
                       if (data.status == 200) {
                           alert("保存成功!!");
                           location.href = "/manage/productSecKill/overview"
                       } else {
                           alert("保存异常");
                       }
                   }
